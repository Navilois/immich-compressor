...
class ImmichClient:
    ...
    async def touch_asset(self, asset_id: str) -> None:
        """Bump the asset's ``updateId`` without changing anything a user would see.

        The sync stream only offers an asset whose ``updateId`` is newer than the client's
        checkpoint, and that column is regenerated by a ``BEFORE UPDATE`` row trigger on
        ``asset``. The trigger carries no ``WHEN`` clause, so writing a column's own
        current value back is enough — measured against PostgreSQL 16: a value-identical
        UPDATE bumps ``updateId``, an UPDATE matching no row does not.

        Two details are load-bearing and neither is obvious:

        * The body must carry a field. ``AssetRepository.update`` skips the UPDATE
          entirely when every value is undefined, so an empty ``PUT`` is a plain read and
          bumps nothing.
        * It must be ``isFavorite``. Of the fields ``PUT /assets/{id}`` accepts, only
          ``isFavorite``, ``visibility`` and ``livePhotoVideoId`` reach the asset table at
          all — the rest are diverted to the exif row. ``livePhotoVideoId: null`` takes an
          unlink path, and ``visibility`` carries more meaning than this needs.

        The value is read immediately before it is written back, so the window in which a
        user could favourite the asset and have it overwritten is one round trip.
        """
        detail = await self.get_asset(asset_id)
        await self.update_asset(asset_id, UpdateAssetFields(is_favorite=bool(detail.is_favorite)))
